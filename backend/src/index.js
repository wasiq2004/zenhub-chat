...
// Behind Caddy (TLS) → nginx → here = 2 reverse-proxy hops. Trust them so
// req.ip / req.secure / X-Forwarded-* reflect the real client. Without this,
// the rate-limiter buckets every request under the nginx container IP and
// express-rate-limit emits a validation error when it sees X-Forwarded-For.
app.set('trust proxy', 2);

// Log (don't silently die) on unexpected async failures. With
// restart: unless-stopped a crash here would otherwise loop with no diagnostics.
...
// Only allow the localhost bypass outside production. In production (the VPS)
// CORS is restricted to the explicit CORS_ORIGIN domain so a page on a user's
// own machine can't make credentialed cross-origin calls to the live API.
...
async function start() {
  ...
  const shutdown = async (sig) => {
    ...
    // Force-exit if a hung connection prevents clean drain within 15s.
    const force = setTimeout(() => {
      console.error('[ForgeChat] drain timed out, forcing exit');
      process.exit(1);
    }, 15000).unref();
    try {
      ...
      console.error('[ForgeChat] shutdown error:', err.message);
    } finally {
      clearTimeout(force);
      process.exit(0);
    }
